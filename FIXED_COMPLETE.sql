-- COMPLETE FIX FOR WORKOUT_SESSIONS TABLE (Fixed version)
-- Run ALL of this in Supabase SQL Editor

-- 1. Drop the existing table and recreate it properly
DROP TABLE IF EXISTS workout_sessions CASCADE;

-- 2. First check if workout_clients uses UUID or TEXT for id
-- If it's UUID, we need to match that type
CREATE TABLE workout_sessions (
  id TEXT PRIMARY KEY,
  title TEXT NOT NULL,
  plan JSONB NOT NULL DEFAULT '{}'::jsonb,
  client_id UUID REFERENCES workout_clients(id) ON DELETE SET NULL,  -- Changed to UUID to match workout_clients
